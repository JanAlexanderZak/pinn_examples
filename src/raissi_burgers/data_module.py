    ...
    def __getitem__(self, idx: int) -> Tuple[torch.utils.data.dataset.TensorDataset]:
        ...
        Returns:
            Tuple[torch.utils.data.dataset.TensorDataset]:
                Tuple of datasets to for training_step.
        """
        return tuple(self.datasets[0][i][idx] for i in range(len(self.datasets[0])))

    def __len__(self) -> int:
        """ The total dataset length to determine batch per dataset
...
class RaissiPINNDataModule(pl.LightningDataModule):
    ...
    def update_collocation_points(self, new_points):
        """ Add new collocation points to the training dataset.
        Used by ResidualAdaptiveRefinement callback.

        Args:
            new_points: numpy array of shape (N, 2) with new (x, t) points.
        """
        existing = self.dataset_train.tensors[0]
        new_tensor = torch.Tensor(new_points)
        combined = torch.cat([existing, new_tensor], dim=0)
        self.dataset_train = torch.utils.data.TensorDataset(combined)
