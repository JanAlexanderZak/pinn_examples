""" DataModule for projectile trajectory.
"""
import os
...
class ConcatDatasets(torch.utils.data.Dataset):
    """ ConcatDatasets joins different datasets (e.g. collocation points, IC)
...
class ProjectilePINNDataModule(pl.LightningDataModule):
    def __init__(
        self,
        path_to_data: str,
        args: Callable,
    ...
    def load_data(path_to_data) -> tuple:
        return (
            np.load(os.path.join(path_to_data, "t_train.npy")),
            np.load(os.path.join(path_to_data, "x_train_IC.npy")),
            np.load(os.path.join(path_to_data, "y_train_IC.npy")),
            np.load(os.path.join(path_to_data, "t_star.npy")),
        )
    ...
    def setup(self) -> None:
        t_train, x_train_IC, y_train_IC, t_star = (
            self.load_data(self.hparams.path_to_data)
        )

        self.column_names = list(["t"])
        self.target_names = list(["x", "y"])
        self.in_features = 1   # model input: time t
        self.out_features = 2  # model output: position (x, y)
