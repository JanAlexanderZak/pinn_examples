""" DataModule for Euler-Bernoulli beam deflection.
"""
import os
...
class BeamPINNDataModule(pl.LightningDataModule):
    def __init__(
        self,
        path_to_data: str,
        args: Callable,
    ...
    def setup(self) -> None:
        x_train, x_train_BC, y_train_BC, x_star = (
            self.load_data(self.hparams.path_to_data)
        )

        self.column_names = list(["x"])
        self.target_names = list(["w"])
        self.in_features = 1   # model input: x position only
        self.out_features = 1  # model output: deflection w
